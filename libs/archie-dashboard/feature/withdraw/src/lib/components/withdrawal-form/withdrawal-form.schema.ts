...
export const getWithdrawSchema = (maxAmount: BigNumber) =>
  yup.object({
    withdrawAmount: yup
      ...
      .test('min', 'dashboard_withdraw.form.amount_min_error', (val: string | undefined) => val === undefined 
        ? false 
        : BigNumber(val).isGreaterThan(0))
      .test('max', 'dashboard_withdraw.form.amount_max_error', (val: string | undefined) => val === undefined 
        ? false 
