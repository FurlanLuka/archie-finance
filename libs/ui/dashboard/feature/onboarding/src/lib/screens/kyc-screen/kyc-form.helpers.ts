...
export const addAddress = (place: GooglePlace): Partial<Address> => {
  const streetNumberComponent = place.address_components.find((item) =>
    item.types.includes('street_number'),
  );
  const streetNameComponent = place.address_components.find((item) =>
    item.types.includes('route'),
  );
  const localityComponent = place.address_components.find((item) =>
    item.types.includes('locality'),
  );
  const countryComponent = place.address_components.find((item) =>
    item.types.includes('country'),
  );
  const postalCodeComponent = place.address_components.find((item) =>
    item.types.includes('postal_code'),
  );
  const postalTownComponent = place.address_components.find((item) =>
    item.types.includes('postal_town'),
  );
  const regionComponent = place.address_components.find((item) =>
  ...
  const addr: Partial<Address> = {
    ...
    addressLocality: localityComponent
      ? localityComponent.short_name
...
export const getAddressError = (errors: FieldErrors<Address>) =>
