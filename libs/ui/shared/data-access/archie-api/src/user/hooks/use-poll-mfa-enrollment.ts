...
} from '../api/get-mfa-enrollment';

export const MFA_ENROLLMENT_QUERY_KEY = 'mfa_enrollment';

export const usePollMfaEnrollment = (
  isEnabled: boolean,
): QueryResponse<GetMfaEnrollmentResponse> => {
  const queryClient: QueryClient = useQueryClient();

  return useExtendedQuery(
    MFA_ENROLLMENT_QUERY_KEY,
    async (accessToken: string) => getMfaEnrollment(accessToken),
    {
