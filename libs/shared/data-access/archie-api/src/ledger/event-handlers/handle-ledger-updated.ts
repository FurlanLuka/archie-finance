...
export const handleLedgerUpdatedEvent = (event: LedgerUpdatedWsEvent): void => {
  queryClient.setQueryData<Ledger | undefined>(LEDGER_QUERY_KEY, (data): Ledger | undefined => {
    if (data !== undefined) {
      const updatedAccounts = mergeLedgerWithWsUpdate(data.accounts, event.data.ledgerAccounts);
      data.accounts.reduce((newAccounts: LedgerAccountData[], account: LedgerAccountData) => {
