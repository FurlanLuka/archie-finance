...
export default async function runExecutor(
  ...
  execSync(
    `docker buildx build -f ${projectConfig.root}/Dockerfile.ci -t ${options.ecrRegistry}/${options.ecrRepository}:${versionTag} --cache-from type=gha,scope=${options.sha} --push .`,
  );
