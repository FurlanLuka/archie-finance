import { GET_LOAN_BALANCES_RPC, SERVICE_QUEUE_NAME } from '@archie/api/peach-api/constants';
import { GetLoanBalancesPayload, GetLoanBalancesResponse } from '@archie/api/peach-api/data-transfer-objects/types';
...
export class LoanBalancesController {
  ...
  @UseGuards(AuthGuard)
  @ApiBearerAuth()
  @ApiErrorResponse([NotFoundException, BorrowerNotFoundError, CreditLineNotFoundError])
...
export class LoanBalancesQueueController {
  ...
  @RequestHandler(GET_LOAN_BALANCES_RPC, LoanBalancesQueueController.CONTROLLER_QUEUE_NAME)
  async getLoanBalances(payload: GetLoanBalancesPayload): Promise<RPCResponse<GetLoanBalancesResponse>> {
    try {
      const data = await this.loanBalancesService.getLatestLoanBalance(payload.userId);
