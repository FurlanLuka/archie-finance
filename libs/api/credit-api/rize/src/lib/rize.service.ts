...
import { CARD_ACTIVATED_TOPIC, TRANSACTION_UPDATED_TOPIC } from '@archie/api/credit-api/constants';
import { QueueService } from '@archie/api/utils/queue';
import { GET_USER_EMAIL_ADDRESS_RPC, GET_USER_KYC_RPC } from '@archie/api/user-api/constants';
import {
  RizeTransaction,
  Transaction,
  TransactionResponse,
...
export class RizeService {
  ...
  constructor(
    ...
    this.rizeMessagingQueueClient = rizeApiService.connectToRizeMessagingQueue();
  ...
  private async handleTransactionsEvent(message: TransactionEvent) {
    ...
    if (![TransactionType.fee, TransactionType.credit].includes(transactionDetails.type)) {
      ...
      const transaction: RizeTransaction = await this.rizeApiService.getTransaction(transactionId);
  ...
  public async getVirtualCard(userId: string): Promise<CardResponseDto> {
    ...
    const debitCard: DebitCard | null = await this.rizeApiService.getDebitCard(customer.uid);
    this.rizeValidatorService.validateDebitCardExists(debitCard);

    const debitCardAccessToken: DebitCardAccessToken = await this.rizeApiService.getDebitCardAccessToken(debitCard.uid);

    const image: string = await this.rizeApiService.getVirtualCardImage(debitCardAccessToken);

    return {
      image,
      status: debitCard.locked_at !== null ? CardStatus.frozen : CardStatus.active,
      freezeReason: debitCard.lock_reason ?? null,
    };
  }

  public async getTransactions(userId: string, page: number, limit: number): Promise<TransactionResponse> {
    const customer: Customer | null = await this.rizeApiService.searchCustomers(userId);
    this.rizeValidatorService.validateCustomerExists(customer);

    const rizeTransactions: RizeList<RizeTransaction> = await this.rizeApiService.getTransactions(
      customer.uid,
      page,
      limit,
      ['queued', 'pending', 'settled', 'failed'],
    );

    const transactions: Transaction[] = rizeTransactions.data.map((txn: RizeTransaction) => ({
      status: txn.status,
  ...
  public async createUser(userId: string, userIp: string): Promise<void> {
    const existingCustomer: Customer | null = await this.rizeApiService.searchCustomers(userId);
    ...
    const emailAddressResponse: EmailAddress = await this.queueService.request<EmailAddress, GetEmailAddressPayload>(
    ...
    const customerId: string =
      existingCustomer !== null
        ...
        : await this.rizeApiService.createCustomer(userId, emailAddressResponse.email);
    ...
    try {
      complianceWorkflow = await this.rizeApiService.createCheckingComplianceWorkflow(customerId);
    } catch {
      complianceWorkflow = await this.rizeApiService.getLatestComplianceWorkflow(customerId);
    }

    await this.acceptAllDocuments(customerId, userId, userIp, complianceWorkflow);

    await this.rizeApiService.createCustomerProduct(customerId, complianceWorkflow.product_uid);
  ...
  private async acceptAllDocuments(
    ...
    if (complianceWorkflow.current_step_documents_pending.length !== 0) {
      const docs: ComplianceDocumentAcknowledgementRequest[] = complianceWorkflow.current_step_documents_pending.map(
        (pendingDocument) =>
          this.rizeFactoryService.createAcceptedComplianceDocument(pendingDocument.uid, userIp, userId),
      );

      const updatedComplianceWorkflow: ComplianceWorkflow = await this.rizeApiService.acceptComplianceDocuments(
        customerId,
        complianceWorkflow.uid,
        docs,
      );

      await this.acceptAllDocuments(customerId, userId, userIp, updatedComplianceWorkflow);
    }
  }

  public async loadFunds(userId: string, customerId: string): Promise<void> {
    const credit = await this.queueService.request<GetLoanBalancesResponse, GetLoanBalancesPayload>(
  ...
  public async updateAvailableCredit(credit: CreditBalanceUpdatedPayload): Promise<void> {
    if (credit.paymentDetails?.type === PaymentType.purchase) {
      return;
    }

    const updateResult: UpdateResult = await this.lastDebitCardUpdateMetaRepository.update(
      {
    ...
    const customer: Customer | null = await this.rizeApiService.searchCustomers(credit.userId);
    this.rizeValidatorService.validateCustomerExists(customer);
    const pendingTransactions: RizeList<RizeTransaction> = await this.rizeApiService.getTransactions(
      customer.uid,
      0,
      100,
      ['pending', 'queued'],
    ...
    if (credit.availableCreditAmount > cardBalance) {
      await this.rizeApiService.loadFunds(customer.uid, credit.availableCreditAmount - cardBalance);
    }

    if (credit.availableCreditAmount < cardBalance) {
      await this.rizeApiService.decreaseCreditLimit(customer.uid, cardBalance - credit.availableCreditAmount);
    }
  }
