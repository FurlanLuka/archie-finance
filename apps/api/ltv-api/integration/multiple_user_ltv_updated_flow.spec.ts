...
describe('Ltv api tests', () => {
  ...
  describe('Receive ledger accounts updated event for multiple ledgers (users)', () => {
    beforeAll(setup);
    afterAll(cleanup);
    afterEach(() => {
    ...
    it(`should keep ltv for all as 0 as credit line was not created yet`, async () => {
      await app.get(LtvQueueController).ledgersUpdated([ledgerUpdatedPayload]);
    ...
    it(`should reset ledger value`, async () => {
      ...
      await app
        .get(LtvQueueController)
        .ledgersUpdated([ledgerAccountUpdatedPayload]);
